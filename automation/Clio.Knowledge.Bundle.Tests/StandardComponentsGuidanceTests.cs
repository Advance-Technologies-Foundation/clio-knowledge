...
public sealed class StandardComponentsGuidanceTests
{
    ...
    [Test]
    [Description("Keeps the crt.TagSelect case stated as the opposite of the attachments case: template-provided, preprocessor-wired, never hand-wired on the page.")]
    public void StandardComponentsGuidance_ShouldOwnTheTagSelectShape()
    {
        // Arrange
        string guidance = ReadGuide();
        ...
        guidance.Should().Contain("{ \"type\": \"crt.TagSelect\", \"recordId\": \"$Id\", \"name\": \"TagSelect\" }",
            ...
            .And.Contain("There is no merge-vs-insert decision",
                because: "both measured templates ship the component — including the one that ships "
                    + "neither the feed nor the attachments list — so STEP 1 does not apply to tags")
        ...
        guidance.Should().Contain("You MUST NOT hand-wire `items`, `listItems` or the CRUD outputs",
                because: "this is the instruction that inverts STEP 3 to STEP 5, and an agent reading "
                    + "the attachments steps first will otherwise generalise the wrong way")
    ...
    public void StandardComponentsGuidance_ShouldNotPrescribeTheTagJunctionItDidNotVerify()
    {
        // Arrange
        string guidance = ReadGuide();
        ...
        guidance.Should().Contain("needs NO per-object junction schema",
            ...
            .And.Contain("does NOT tell you to create an `<Entity>InTag` schema",
                because: "writing a fix instruction nobody verified is the failure mode this whole "
    ...
    public void PageModificationRouter_ShouldRouteFeedAndAttachmentsEditsToTheOwningGuide()
    {
        ...
        router.Should().Contain("crt.TagSelect",
            because: "tags are in this guide's scope and produce the same user-visible symptom as "
                + "attachments; a GATE row that never says so routes a tag edit nowhere");
    }
