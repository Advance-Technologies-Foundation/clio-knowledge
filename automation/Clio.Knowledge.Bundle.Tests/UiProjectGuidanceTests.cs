...
public sealed class UiProjectGuidanceTests
{
    ...
    public void UiProjectGuidance_ShouldRequireARealSpec_WhenRunningAngularTests()
    {
        ...
        guidance.Should().Contain("`npm test` or `ng test`",
            ...
            .And.Contain("Inspect the generated `package.json` test script and `angular.json` test builder",
                because: "runner-specific advice must be selected from the generated project rather than an assumed clio version")
            .And.Contain("For `@angular-builders/jest:run`, the builder owns Angular test-environment initialization",
                because: "Jest projects must not initialize Angular's test environment twice")
            .And.Contain("Keep `import '@angular/compiler'` in `setup-jest.ts`, but do not import or call `setupZoneTestEnv` there",
                because: "the legacy duplicate initializer must be removed without dropping the compiler import")
            .And.Contain("Older clio scaffolds can contain that duplicate initializer",
                because: "the published guidance also serves projects generated by pre-fix clio releases")
