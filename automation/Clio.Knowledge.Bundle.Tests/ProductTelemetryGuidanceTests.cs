...
public sealed class ProductTelemetryGuidanceTests
{
    ...
    // Each stage is matched WITH its backticks. Bare "changes_requested" is a substring of
    // "plan_changes_requested", so the unquoted form passed for a stage the article never named.
    private static readonly string[] Stages =
    [
        "`workflow_started`", "`clarification_requested`", "`user_input_received`", "`plan_presented`",
        "`plan_skipped`", "`plan_blocked`", "`plan_changes_requested`", "`plan_approved`",
        "`build_started`", "`work_item_completed`", "`workflow_completed`", "`workflow_failed`",
        "`changes_requested`", "`changes_applied`"
    ];
