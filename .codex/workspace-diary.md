...
Discovery: Creatio core commit `e0d0f98b80c8fd26e305804c7cb3242b76baf072` shows `AppEventLoader` creates a fresh instance for every dispatch; `AppEventDispatcher` catches and logs per-listener failures and continues. `AppEventContext` carries application state only. Both .NET Framework and .NET Core raise app start/end and session start/end, but their surrounding service/session ordering differs, so portable guidance cannot promise a user identity or one host's ordering. `OnSessionExpired` is not part of `IAppEventListener`.
