...
## 2026-08-20 – ENG-95429 guidance: the accuracy lens caught a Blocker my own fix introduced
Discovery: JsonDiffApplier.Merge NEVER throws — the strip is warn-and-drop, and the only throw in the file is NotContainerItemInsertException at JsonDiffApplier.cs:620, inside Insert. My previous fix had attributed that build failure to a merge on itemLayout, importing it from web-to-mobile-conversion.md:293-295. Removed the claim.
Discovery: an insert into a single-element slot throws ONLY when the property is null or absent. When it already holds an object, `parent is JObject && itemInfo is not null` matches and the insert REPLACES it (JsonDiffApplier.cs:615-618). So "an insert cannot address it, a merge is the only route" was wrong for the populated case, and the guide had left that case with no working route at all. clio's own runtime message carried the correct qualifier ("null or absent"); the guide had dropped it.
Discovery: the two cases are opposites — populated slot: merge stripped, insert replaces; null/absent slot: insert throws, merge is the only route. A single-element slot is not a carve-out from the strip rule, it just has a different remedy on each side.
Discovery: crt.MenuItem is NOT in the mobile component registry (35 componentTypes, it is absent) — the worked example invented a crt.* type, which the same guide's NO INVENTION rule forbids. Replaced with a placeholder plus a pointer to get-component-info.
OPEN: web-to-mobile-conversion.md:293-295 still attributes "is not a container for other items" to a merge of itemLayout onto the parent crt.List. Against clio's clone of the applier that belongs to the insert. The claim came from an ENG-94937 stand investigation, so the client may differ from the clone — not rewritten here; needs settling on a stand.
Files: guidance/mcp/guides/platform/mobile/page-modification.md, bundle-source.json, automation/Clio.Knowledge.Bundle.Tests/MobileOperationShapeRuleTests.cs
Impact: reading the applier is not enough when a sibling guide records a stand observation — but repeating that observation as mechanism, without checking which operation it belongs to, is how the wrong half gets propagated.
