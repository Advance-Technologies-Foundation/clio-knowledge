...
Discovery: PublishedContentDigest could NOT be regenerated on this machine - the bundle automation targets net10.0 and only SDK 8.0.423 is installed; the constant is marked with a TODO and the test failure message supplies the correct digest on any .NET 10 machine/CI.
...
Impact: get-guidance name=process-modeling will teach AI callers the full sendEmail contract once published; publish requires the digest regeneration first.
