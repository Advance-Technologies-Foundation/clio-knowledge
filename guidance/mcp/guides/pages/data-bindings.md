...
- Applying a local binding artifact: the artifact is a TRANSFERABLE SOURCE, not an applied row. Exactly two things materialize it. (1) Package installation — `push-pkg` / `push-workspace` with `install-package-data`, which is what carries the binding to the NEXT environment. (2) The DB-first tools — `create-data-binding-db` then `upsert-data-binding-row-db` — which write the live row AND register it in the package, i.e. they are a strict superset of writing the row with `odata-create`. `pkg-to-db` does NOT apply data: it registers package DEFINITIONS from the file system into the configuration database, and installs no binding rows into any business table. Do not read its success as the row having landed; read the row back (`read-data-binding-db`, or `odata-read` on the target schema).
- Applying in file system mode (FSM): FSM is where this goes wrong most often, because the workspace flow that ends at a local artifact and the FSM flow that ends at `pkg-to-db` look like the same flow and are not. In FSM the shipped workspace `AGENTS.md` forbids `push-workspace` (it rebuilds from the stale database copy and overwrites a good file-system build), and `pkg-to-db` does not apply data — so neither of the two obvious moves applies the row. Apply the identical values with `create-data-binding-db` / `upsert-data-binding-row-db` against the linked environment, then read back. Keep the local `Data/` artifact as the transfer source for the next environment.
