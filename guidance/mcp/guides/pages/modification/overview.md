...
- Modifying an existing component: edit the operation that introduces it. Identify own-body inserts via `ownBodySummary.viewConfigDiffOps` (`operation: insert`) and edit their `values`; reserve `merge`/`move`/`remove` for parent-introduced elements. Two different failures follow from ignoring this, and they differ by write mode:
  ...
  * In `mode: "append"`, the insert is NOT discarded — identity includes the operation, so both survive the merge. But surviving is not the same as applying: see "operation apply order" below.
...
- What to do: fold the change into the `insert`'s own `values`, or use `set`. Pairing `merge` or `move` with your own `insert` does not compose — but a `move` plus a `merge` on a component you did NOT insert is fine and both apply.
