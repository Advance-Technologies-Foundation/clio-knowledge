...
- `update-page` reports all of this as advisory warnings (never blocking) — it cannot make the operations apply, because a single body cannot see whether a parent schema inserts the same name. Eight pairs are warned about, for one component name: `merge`, `move`, or element `remove` beside your own `insert`; `merge` or `move` beside an element `remove`; `merge` or a property `remove` beside a `set`; and a property `remove` beside an element `remove` (that last one is not warned when an `insert` for the same name rescues it by re-creating the element first).
