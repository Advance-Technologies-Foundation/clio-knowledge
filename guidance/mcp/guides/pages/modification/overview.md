...
- A grid that renders only `crt-data-grid-placeholder` after a save is in the platform's pre-load state, not broken — no query for its entity has been issued yet. `related-list` owns the verification rule ("Verifying a list in the browser").
