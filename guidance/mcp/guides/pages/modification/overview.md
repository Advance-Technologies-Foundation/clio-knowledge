...
- Merge rules (append mode):
  * `viewConfigDiff` — concat, then replace ONLY on a full identity match: `operation` + `name`, plus, for a `remove` or a `set`, whether it carries a `properties` array (incoming wins, in place). Every existing operation your fragment does not collide with is preserved, INCLUDING a second operation on a component you already target — a `move` and a `merge` for one name are both valid and both survive. One exception: if the page carries an identity more than once and your fragment supersedes it, only the first occurrence is replaced and the rest are dropped; `update-page` warns and names the component when that happens. This identity rule ships in clio 8.1.0.122 and later; an earlier clio deduped `viewConfigDiff` by `name` alone, so on one of those a fragment's `merge` REPLACES an existing `insert` for that name instead of joining it.
  * `handlers` — concat + dedupe by `request` string (incoming wins)
  * `converters` — merge object by key (incoming wins)
  * `validators` — merge `SCHEMA_VALIDATORS` object by validator type key (incoming wins); the final merged web body is rejected when a custom validator reference has no matching local declaration, except built-in `crt.*` validators
  * `viewModelConfigDiff` / `modelConfigDiff` — plain concat (no dedupe)
- Validator merging was verified through the real MCP save/read-back path on Creatio 10.1.585.0 (.NET 8, PostgreSQL) for clio issue 1249: both the `viewModelConfigDiff` binding and JavaScript validator factory remained in the saved schema body.
...
- Modifying an existing component: edit the operation that introduces it. Identify own-body inserts via `ownBodySummary.viewConfigDiffOps` (`operation: insert`) and edit their `values`; reserve `merge`/`move`/`remove` for parent-introduced elements. Two different failures follow from ignoring this, and they differ by write mode:
  * In `mode: "replace"`, authoring a same-`name` `merge`/`move`/`remove` with no `insert` kept DISCARDS the insert, so unless a parent schema inserts that name the component is orphaned (disappears at runtime).
  * In `mode: "append"` on clio 8.1.0.122 and later, the insert is NOT discarded — identity includes the operation, so both survive the merge. But surviving is not the same as applying: see "operation apply order" below. On an earlier clio the append dedupes by `name` alone, so the transform REPLACES the insert and you get the `mode: "replace"` outcome above instead.
  `update-page`/`sync-pages` warn (advisory, never block) on both shapes.
...
operation apply order — preserved is not the same as applied
- The platform differ does NOT apply `viewConfigDiff` in the order you write it. It buckets operations by verb and runs whole groups in a fixed sequence: all `merge`, then the position pipeline (`remove`/`insert`/`move`), then property-`remove`, then `set`. Array order is honoured only WITHIN a group (two `merge` ops for one name apply in sequence); across groups, reordering the array changes nothing.
- The general rule: give one component name two operations whose groups run in sequence, and the earlier one resolves against a state the later one has not produced yet. A `merge` or a `move` aimed at a component the SAME body inserts resolves against a base that does not contain it, and is discarded. Only `set` and a property `remove` run after the insert pipeline — but only ONE of them composes with your own `insert`. A property `remove` does: it strips the named keys off the element the insert just built. A `set` does NOT: it removes the element and rebuilds it from its OWN `values`, keeping only the insert's `index` and `propertyName`, so everything else you wrote into the insert is lost. Use `set` INSTEAD of the `insert`, never beside it — and clio does not warn about this pair, because the insert is not inert, only overwritten. The two cannot be combined either, see the last bullet.
- An element `remove` beside an `insert` for one name depends on where the component comes from. If a PARENT schema introduces it, this is the replace-an-inherited-component idiom and BOTH operations apply (`set` expresses the same thing in one operation). If THIS body inserts it, the remove is resolved against the base before any insert has run, finds nothing, and is skipped — so it cannot delete what this body adds, and both survive.
- One shape does not follow from group order at all: a `move` beside an element `remove` for one name. The position pipeline opens by filtering the move list against the remove list and drops EVERY move whose name matches ANY remove — before anything is applied, and whether or not the remove itself resolves. The move is always lost; whether the component is also deleted depends on the remove resolving. Drop the remove if you meant to relocate.
- What to do: fold the change into the `insert`'s own `values`, or replace the `insert` with a `set` that carries the complete configuration. Pairing `merge` or `move` with your own `insert` does not compose — but a `move` plus a `merge` on a component you did NOT insert is fine and both apply.
- `update-page` reports all of this as advisory warnings (never blocking) in clio 8.1.0.122 and later — it cannot make the operations apply, because a single body cannot see whether a parent schema inserts the same name. An earlier clio saves the same body silently; everything above still describes what the platform does with it, because the apply order is the differ's and does not depend on the clio that wrote the schema. Eight pairs are warned about, for one component name: `merge`, `move`, or element `remove` beside your own `insert`; `merge` or `move` beside an element `remove`; `merge` or a property `remove` beside a `set`; and a property `remove` beside an element `remove` (that last one is not warned when an `insert` for the same name rescues it by re-creating the element first).
...
- A grid that renders only `crt-data-grid-placeholder` after a save is in the platform's pre-load state, not broken — no query for its entity has completed yet (it may not have been issued at all, or it may still be in flight). `related-list` owns the verification rule ("Verifying a list in the browser"), including when the placeholder IS a real failure.
