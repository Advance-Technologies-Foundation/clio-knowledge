...
- `visible` is a view-engine property available on web and mobile view elements alongside `type` and element-specific properties. It accepts `true`, `false`, or a binding expression (e.g. `"$SomeAttr | crt.InvertBooleanValue"`); availability does not imply identical navigation effects for every component.
- WEB tabs (`crt.TabPanel`, `mode: "tab"`): a child `crt.TabContainer` with `visible: false` removes that tab's header/choice from the tab strip, not merely its inactive content. Do NOT mark non-default tabs invisible to select the initial tab. Keep every intended tab choice visible; retain conditional visibility only when the tab should actually be unavailable. If every child is hidden, there is no tab choice to click. A successful save or `validate-page` result does not prove the intended headers render.
- Control selection on the parent panel with `selectedTab: {"value": "<TabContainerName>"}` (the child's `name`, not its caption), or the zero-based `selectedTabIndex`; do not set conflicting values for both. Selection does not reveal a hidden child. Saved user-profile selection can override an initial default: verify the default on a fresh panel/profile and separately verify reopening an existing selection. After saving, count the intended headers in the rendered tab strip and click each to check its content. These tab observations were verified on web Creatio `10.1.585.0`; mobile and `mode: "toggle"` were not tested. Evidence: [tab visibility lab](https://github.com/Advance-Technologies-Foundation/clio-knowledge/issues/159#issuecomment-5639963010).
