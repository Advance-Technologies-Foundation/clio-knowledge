...
- For how to insert and configure any `crt.*` component — including the child-collection slots a container exposes — `get-component-info` is the authoritative source. Call it for the exact type and build the insert from its response and embedded `documentation`; do not author component shape from this guide or from memory. `get-component-info` stays the authoritative source for components. One scope note: a component that appears in the `elementMap` of `get-mobile-page-conversion-guide` for THIS page has its placement owned by ELEMENT PLACEMENT IS AUTHORITATIVE (`get-guidance` `freedom-page-web-to-mobile-conversion`) — take that entry's parent verbatim. A component with NO `elementMap` entry — e.g. one added later to an already-converted page — falls outside that rule, so `get-component-info`'s parent/container guidance still applies.
