...
Verifying a list in the browser — a placeholder is NOT a failure
A `<div data-qa="crt-data-grid-placeholder">` inside the grid's container is the platform's normal
PRE-LOAD state for EVERY Freedom UI list — an inserted detail, an out-of-the-box detail, and a
section `*_ListPage` grid alike. Measured on Creatio 10.0.0.934 (.NET Framework); treat the rule as
applying to 10.0.0.9xx and later, and re-verify on an older build before relying on it there. While it shows, the grid has no rows yet for one of two reasons:
its `DataService/json/SyncReply/SelectQuery` has not been issued AT ALL (the grid is outside the
viewport of its scroll container), or that query is still in flight. So the triple "placeholder in
the DOM + no JavaScript error + no failing network request" is exactly what a list that has not
loaded YET looks like. It is NOT evidence that the configuration is wrong, and an agent that reads
the DOM right after the save and stops there reports a WORKING list as broken.

The placeholder is replaced when BOTH hold: the grid has entered the viewport of its own scroll
container (on a FormPage that is the page `MainContainer`, NOT `window` — scrolling the window does
nothing), and the data load has returned. Waiting alone is enough only for a grid that is ALREADY in
the viewport, and even then it takes tens of seconds on a slow instance. A grid left below the fold
stays a placeholder for as long as it stays there — through a save, a `compile-creatio`, and an
environment restart (observed on 10.0.0.934) — because nothing ever requests its data.

Verify a list this way, and do not report failure before all three steps:
1. Reload the page and bring the grid into view — set `scrollTop` on the page `MainContainer`, or
   scroll the mouse over the page body. Confirm `getBoundingClientRect().top` is inside the viewport.
   `MainContainer` is the FormPage scroll container; on a section `*_ListPage` the grid fills the page
   and is normally already in view, so do not hunt for a named container there — read the grid's own
   `getBoundingClientRect()` and, if it is off-screen, scroll its nearest ancestor whose
   `scrollHeight` exceeds its `clientHeight`.
2. Wait for the `SelectQuery` on the grid's `entitySchemaName`. Tens of seconds between viewport
   entry and the first row is normal on a slow instance (cloud demo, PostgreSQL), so poll instead of
   sampling once.
3. Read the rows out of the grid container. Absence of the placeholder is not the same as rows.
   This step is also where the rule TERMINATES — name the failure, in either direction:
   - once the `SelectQuery` for the grid's `entitySchemaName` has been issued AND has returned, the
     grid's content is a real result. An empty grid at that point is a genuine finding, not a
     pre-load state: either the child set really is empty, or the configuration is wrong (a missing
     or wrong `dependencies` entry, an `attributePath` pointing at the `...Id` form instead of the
     bare reference column — see Common mistakes below).
   - if NO `SelectQuery` for that entity appears at all after the grid is confirmed inside its scroll
     container's viewport and polled for tens of seconds, that ABSENCE is itself the broken signal —
     typically the slot-initialization footgun above ("Item ... is not a container for other items")
     or a grid that was never wired to a data source. Report it as a failure; do not keep waiting.
   In short: keep reporting "still loading" only while the query is outstanding. Once it has returned,
   or once it has demonstrably never been issued, you have a verdict and must give it.

Evidence (clio issue #1420, Creatio 10.0.0.934, .NET Framework): an inserted `crt.DataGrid` over
`Contact` on `Accounts_FormPage` stayed a placeholder with no query for 30 s at `rect.top` 1051 in a
700 px viewport, and loaded 15+ rows once scrolled to `rect.top` 498. A section list generated by
`create-app-section` behaved identically — placeholder and no rows ~20 s after load, full rows by
~45 s — even though it sits at the top of the page.
...
- Declaring the list broken because the grid container holds only `crt-data-grid-placeholder`. That
  is the pre-load state, not a failure — see "Verifying a list in the browser". Scroll the grid into
  its scroll container's viewport and wait for the `SelectQuery` before you judge it.