...
section `*_ListPage` grid alike. While it shows, the grid has no rows yet for one of two reasons:
...
stays a placeholder indefinitely — through a save, a `compile-creatio`, and an environment restart —
because nothing ever requests its data.
