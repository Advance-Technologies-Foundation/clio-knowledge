...
- Primary legacy message-template content is exposed as `EmailTemplate.Subject`, `Body`, `TemplateConfig`, `ConfigType`, and `IsHtmlBody`. Translations are separate `EmailTemplateLang` rows selected by `language-id` (`SysLanguage.Id`). `IsHtmlBody` is the exception to the primary-only split: an update writes it to both the primary `EmailTemplate` row and the selected `EmailTemplateLang` row.
...
- Echo the language identity back to `update-email-template` exactly as `get-email-template` returned it. The guarded-creation checksum is computed from the read-side normalized `language-id` but from the raw write-side arguments, so a braced or upper-case GUID, or a `language` sent alongside `language-id` for `legacy`, fails with "Email content changed after it was read" on a creation that never conflicted with anything. MUST pass the returned `language-id` verbatim, and MUST NOT send `language` together with `language-id` for `legacy`.
...
- For `legacy`, send only fields intentionally changed, and send at least one of `subject`, `body`, `template-config`, `config-type`, or `is-html-body`. Omitted values are preserved. `is-html-body` is accepted and is written to both the primary row and the selected `EmailTemplateLang` row.
- `config-type` applies only to the primary `EmailTemplate` row: the tool returns a failure when `config-type` and `language-id` arrive together, so MUST NOT combine them. `language-id` applies only to message-template translations.
...
- Preserve the source format. Copy Beefree to Beefree and legacy to legacy. If the target lacks the selected variant, use the target's returned `exists=false` checksum to create it, passing the target's language identity exactly as its own read returned it (see "Read before write").
