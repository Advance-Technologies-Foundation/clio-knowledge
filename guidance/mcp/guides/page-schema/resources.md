...
SAME RULE ON THE SOURCE PATH (`push-workspace`). This is one rule for every deploy path, not an `update-page` quirk: author workspace page sources to it too — resource-bound strings plus their resource registration (B2 declarations in `metadata.json` and the culture resource files; `localizable-values` owns those details). `push-workspace` does NOT reject an inline literal, because most stock and existing repository schemas still carry them. It installs the page and, in clio builds that carry the change for [Clio #1639](https://github.com/Advance-Technologies-Foundation/clio/issues/1639), prints one warning per page schema naming the schema, its package, and the offending `<node>.<property>` elements — exactly the nodes `update-page` would reject. Earlier builds install such a page without any message. Treat the warning as a defect to fix, not as noise: a page that deployed from git with a literal is refused the next time anyone edits it through `update-page`.
