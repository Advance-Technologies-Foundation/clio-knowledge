...
- Also use it when the user asks how to test a project generated by `new-ui-project`.
...
- When a generated component's `@CrtOutput()` must trigger a Freedom UI page request, read `page-modification-components`; that guide owns the `@event.detail` request-parameter mapping and handler contract.
...
4. (Optional) After the project is scaffolded, normal Angular workflows apply inside `projects/<projectName>` — install dependencies and run `ng build`; clio does not manage Node tooling.
5. Before testing, inspect the generated `package.json` test script, `angular.json` test builder, and locked builder/preset versions. On the reported Angular 19 stack with `@angular-builders/jest` 19.x and `jest-preset-angular` 14.x, the builder owns Angular test-environment initialization. If `setup-jest.ts` also imports and calls `setupZoneTestEnv`, `npm test` / `ng test` fails before specs with `Cannot set base providers because it has already been called`. Remove that import and call while keeping `import '@angular/compiler'`. Run specs through `npm test` or `ng test`, not a direct `jest` invocation that bypasses the builder.
6. Do not remove an initializer based only on the builder name for other version combinations; inspect that installed builder's setup contract. A version-specific template can use a different test runner, including Karma, and its contract remains authoritative.
7. On that reported Jest stack, a newly generated scaffold contains no `*.spec.ts` files, so Jest reports `No tests found` and exits non-zero until a spec is added. Do not enable `passWithNoTests` merely to make an empty scaffold green; a real spec proves the generated Angular test environment works.
