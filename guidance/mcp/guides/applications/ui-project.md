...
5. When `creatioVersion` is omitted, the current default Angular 19 template uses Jest and contains no `*.spec.ts` files, so Jest reports `No tests found` until a spec is added. Do not enable `passWithNoTests` merely to make an empty scaffold green; a real spec proves the generated Angular test environment works.
6. A version-specific template can use a different test runner, including Karma. Inspect its generated `package.json` test script and `angular.json` test builder, then follow that runner's contract instead of applying the default template's Jest-specific settings.
