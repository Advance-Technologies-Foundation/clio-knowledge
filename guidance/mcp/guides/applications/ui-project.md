...
- Also use it when the user asks how to build or test a project generated by `new-ui-project`.
...
5. Inspect the generated `package.json` test script and `angular.json` test builder before applying runner-specific advice. For `@angular-builders/jest:run`, the builder owns Angular test-environment initialization. Keep `import '@angular/compiler'` in `setup-jest.ts`, but do not import or call `setupZoneTestEnv` there. Older clio scaffolds can contain that duplicate initializer; remove those two legacy lines or regenerate the project with a repaired clio version before trusting test results.
6. A newly generated Jest scaffold contains no `*.spec.ts` files, so Jest reports `No tests found` until a spec is added. Do not enable `passWithNoTests` merely to make an empty scaffold green; a real spec proves the generated Angular test environment works. A version-specific template can use a different test runner, including Karma; follow that runner's contract instead of applying Jest-specific settings.
