...
- The listener class MUST be public and concrete and MUST have a public parameterless constructor. Prefer a `public sealed` class with no explicit constructor.
...
- Creatio catches and logs each listener exception, then continues dispatching other listeners. Throwing does not fail the host lifecycle action and does not give the caller a failure result.
...
| `OnAppEnd(AppEventContext)` | Stop and release package-owned process-local runtime state during graceful host shutdown. | Shutdown MUST be bounded. Do not assume this hook runs after a crash or forced process termination. |
| `OnSessionStart(AppEventContext)` | Observe that the current host raised a session-start event. | Keep work fast and thread-safe. Do not treat the hook as a portable login event or infer a user/session identifier from `AppEventContext`. |
| `OnSessionEnd(AppEventContext)` | Observe that the current host raised a session-end event. | Keep work fast and thread-safe. Do not use it as the only durable logout, cleanup, or commit boundary. |
...
- Do not clear runtime state until workers have actually stopped. If bounded shutdown times out, retain the live state and log an actionable warning so a second runtime cannot start over it.
...
6. Exercise a shutdown timeout in the runtime owner and verify it preserves live state, prevents a duplicate start, and emits a warning.
...
1. Deploy and compile the package, then perform an explicit application restart.
...
4. Perform a graceful application stop and require the package runtime to stop within its configured bound.
...
| --- | --- | --- |
| Listener creation error in `AppEventDispatcher` | the type is not public/concrete or lacks a public parameterless constructor | make the entry point `public sealed` and remove constructor dependencies |
...
| Application/session continues after listener failure | the dispatcher logged and isolated the exception | inspect platform logs; add package-stage logging and make failure state observable in the owner |
...
- Discovery, fresh-instance activation, exception isolation, application/session host ordering, and the application-only event context were verified against Creatio core source at commit `e0d0f98b80c8fd26e305804c7cb3242b76baf072` for both .NET Framework and .NET Core hosts.
