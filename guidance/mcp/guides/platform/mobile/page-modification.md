...
| Web `Rules for viewConfigDiff` section — every bullet EXCEPT the FormPage `DataValueType → component` mapping in the next row (i.e. `operation` / `name` / `parentName` / `propertyName` / `index`, the view-engine `visible` property, and the user-visible string → `$Resources.Strings.*` rule) | Applies in full, except strings inside a `crt.ChartWidget` / `crt.IndicatorWidget` `config` (for example `config.title`): those use `#ResourceString(<key>)#` — see ANALYTICS WIDGETS ON MOBILE |
...
- It governs a body YOU author. A page produced by web-to-mobile conversion follows the placement its
  conversion guide prescribes instead — including inserts into containers that guide's own diff creates,
