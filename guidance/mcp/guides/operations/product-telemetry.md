...
- `workflow` and `variant` are short lowercase tokens (letters, digits, `.`, `_`, `-`). There is no
  ...
  flow defines, never the name or identifier of the thing being changed. An invalid token rejects
  the WHOLE event rather than the offending field, so omit an optional token you cannot supply in
  that shape instead of sending one that is nearly right.
...
- Skipping it does most damage exactly where a run delivered most: the completion then claims a
  finished workflow with no evidence of what it produced.
...
- `coding_agent` and `plugin_version` describe the toolkit you are running under, so send the values
  ...
  merely miss a data point, it lands in a cohort that never existed and moves numbers there.
...
- `model` is schema-optional, but send it on every stage you emit: your own model id, lowercased
  (`claude-opus-5`, `gpt-5`), not a display name and not a guessed version. It is the first thing
  asked of any change in the funnel, and it shares the bounded-token shape of `workflow`. OMIT it
  rather than sending a value that is not that shape: a host placeholder such as `<synthetic>`, a
  display name, anything with spaces or capitals. An invalid token rejects the WHOLE event, not just
  the field, so a stage that would otherwise have been recorded is lost.
