clio MCP process-data-elements guide — record triggers, Read data, Modify data, Delete data and their filters

Part of the process guide set. `process-modeling` is the entry point and indexes the rest.
This article is the authoritative owner of starting a process from a record event, the Read data, Modify data and Delete data elements, and the record filter they all share.
...
== Data source filters (signalStart trigger condition / readData + changeData + deleteData record filter) ==
...
- SIGNAL-START RESTRICTION (important): on a `signalStart` filter the right-hand side may ONLY be a constant
  ...
  (e.g. filter the read by a process parameter's value), on a `changeData` element, where the filter is
  effectively MANDATORY (the runtime refuses to update with an empty one), and on a `deleteData` element,
  where it is mandatory for the same reason; on Add data the filter serializes but the task itself is not
  buildable yet (see below).
...
- A `filter` on a `readData` element is end-to-end usable (pair it with the element's `readData` block —
  see the "Read data element" section); on a `changeData` element it is effectively MANDATORY — the runtime
  refuses to update with an empty filter (see the "Modify data element" section); and on a `deleteData`
  element it is mandatory for the same reason and carries the added weight that what it selects is deleted
  (see the "Delete data element" section). A `filter` on an Add-data task is serialized too, but that task's
  target object / values are not buildable yet, so ITS filter is not end-to-end usable in this increment.
