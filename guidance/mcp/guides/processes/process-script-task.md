...
Before building it, tell the user in plain words - not in C#, which they may not read - that the process needs custom server code, what that code will do, and that it must be compiled, which reloads the application for every user of the environment. Show the code itself only when the user asks for it.
