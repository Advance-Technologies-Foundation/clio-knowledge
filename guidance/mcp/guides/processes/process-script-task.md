...
- Listing a default namespace is harmless: it changes nothing the code sees and owes no compile. An ALIAS on a default namespace is refused: the platform's generator drops such an entry together with its alias, so the alias would not exist at compile time.
