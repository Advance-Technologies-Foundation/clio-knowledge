...
Ask before building it - one question, before the save, never after it. In plain words - not in C#, which the user may not read - say that the task needs custom server code, what that code will do, and that after the save it must be compiled, which takes a few minutes and reloads the application for every user of the environment; then ask whether to add it and compile. Save the ScriptTask only after a yes: a saved ScriptTask whose code does not compile breaks every later compile of its package, for everyone. That yes is the consent for the one compile this save owes - do not ask for it again; a later compile (after a compile error, or for another edit) needs a new one. The same question comes before an edit that changes C# - a body, the methods or a using. When the user asked for the code themselves, skip the why but still ask about the compile before the save. Show the code itself only when the user asks for it. On a no, offer a no-code route from the list above, or leave the ScriptTask out.
