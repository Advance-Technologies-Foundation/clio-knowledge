...
`process-send-email-details`). `template` and `body`/`bodyFormat` in one block
  ...
  sent ALONE never changes the mode — a template element keeps its template (an older CrtProcessBuilder flipped
  it to custom; every version from 1.6.6.14 keeps it), and so does an element with no stored mode that CARRIES a template, because
