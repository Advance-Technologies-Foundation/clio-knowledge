...
- `subProcess` — Sub-process (the BPMN call activity): run ANOTHER process and pass values in and out
  ...
  * NOT to iterate a collection. That is multi-instance, 15 % of shipped usage, and it is REFUSED here
  ...
  means the element does not mirror the called process, which is the usual answer for a multi-instance
  one; `null` means either that the process could not be read or that none is selected yet. `true` is
  the ordinary answer - the read itself converged the element - so it is NOT drift evidence, and it is
  also what a callee declaring NO parameters always reports, the test being one-directional.
