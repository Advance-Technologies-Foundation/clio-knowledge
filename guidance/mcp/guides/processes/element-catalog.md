...
- `subProcess` — Sub-process (the BPMN call activity): run ANOTHER process and pass values in and out
  ...
  * NOT to iterate a collection. That is multi-instance, 15 % of shipped usage, and it is REFUSED here
  ...
  `processCaption`, `multiInstance`, and `inSync`. `inSync` is REAL drift evidence on a COMPILED
  process - describe reads the runtime instance there, which the platform does not converge, so `false`
  after the called process changed means the element is stale and a re-sync is owed. On an UNCOMPILED
  process describe falls back to the design instance, which converges as it loads, and `true` says
  nothing. `null` means the process could not be read or none is selected yet, and `true` is also what a
  callee declaring NO parameters always reports, the test being one-directional.
  `process-parameters` owns this field.
