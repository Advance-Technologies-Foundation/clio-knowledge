...
- Descriptor and modify-operation keys are CASE-SENSITIVE and checked before sending: an undeclared key
  (`lable`, `Label`) would be dropped in silence while the call succeeds, so clio (with ENG-95244) refuses it,
  naming its path and the key meant - or only warns when the environment's CrtProcessBuilder is newer.
