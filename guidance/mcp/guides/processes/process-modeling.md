...
- `callActivity`      Sub-process  — run another process (must start with a Simple start); multi-instance
    over a collection. READ-ONLY here — and note that a sub-process holds its elements INLINE in the same
    schema, so `describe-business-process` and the delete guards do see them.
