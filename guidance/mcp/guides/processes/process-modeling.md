...
- Routing between the three page elements — pick by WHERE the fields live, not by which sounds closest:
  * `preconfiguredPage` — a page YOU designed exists (or will be created) and the user fills it in.
    Choose it when the layout matters or the page must show more than plain fields.
  * `openEditPage` — the user should edit a RECORD of an object on its own edit page (add new, or edit an
    existing one). Choose it when the target is a record, not a form.
  * `autoGeneratedPage` — a throwaway page of fields generated for one step, with no page schema to
    maintain. Choose it for a quick ad-hoc prompt. NOTE its `Buttons` parameter uses a DIFFERENT storage
    shape from `preconfiguredPage`; the two share only the parameter name.
